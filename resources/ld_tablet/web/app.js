...
function waitingOrders(){return (taxiData.orders||[]).filter(function(o){return o.status==='new'||o.status==='dispatch';}).length;}
...
const pages={
 ...
 orders:{title:'Aufträge',render:function(){let orders=taxiData.orders||[];if(!orders.length)return '<div class="list">'+row('Keine offenen Aufträge','bereit')+'</div>';return '<div class="list">'+orders.map(function(o){return row('#'+o.id+' · '+(o.pickup_label||'Abholort unbekannt'),o.status||'offen');}).join('')+'</div>'; }},
 ...
 ratings:{title:'Bewertungen',render:function(){return info('Noch keine Bewertungen verbunden.');}},
 blackboard:{title:'Schwarzes Brett',render:function(){return info('Beiträge folgen im nächsten Modul.');}},
 profile:{title:'Profil',render:function(){return info('Profil wird mit ld_taxi verbunden.');}},
 settings:{title:'Einstellungen',render:function(){return info('Transparenz und Größe folgen als eigenes Modul.');}}
};

function openPage(page,push){if(push===undefined)push=true;let route=pages[page]||pages.home;if(push&&currentPage!==page)historyStack.push(currentPage);currentPage=page;pageTitle.textContent=route.title;topbar.classList.toggle('hidden',page==='home');view.innerHTML=route.render();bindTiles();}
function bindTiles(){document.querySelectorAll('[data-page]').forEach(function(btn){btn.addEventListener('click',function(){openPage(btn.dataset.page);});});}
function post(name){fetch('https://'+GetParentResourceName()+'/'+name,{method:'POST',body:JSON.stringify({})});}
...
homeBtn.addEventListener('click',function(){historyStack=[];openPage('home',false);});
backBtn.addEventListener('click',function(){openPage(historyStack.pop()||'home',false);});
document.addEventListener('keydown',function(e){if(e.key==='Escape')closeTablet();});
setInterval(function(){let now=new Date();timeEl.textContent=now.toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'});},1000);
setInterval(function(){if(!tablet.classList.contains('hidden'))refreshTaxiData();},10000);
openPage('home',false);
